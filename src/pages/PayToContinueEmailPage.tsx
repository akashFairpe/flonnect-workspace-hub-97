import React from 'react';
import { Button } from '@/components/ui/button';
import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card';
import { Badge } from '@/components/ui/badge';
import { ArrowLeft, CreditCard, LogIn, XCircle } from 'lucide-react';
import { Link } from 'react-router-dom';

export default function PayToContinueEmailPage() {
  return (
    <div className="min-h-screen bg-gray-50">
      {/* Header */}
      <div className="bg-white shadow-sm border-b">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <div className="flex items-center gap-4">
            <Link to="/email-setup">
              <Button variant="ghost" size="sm">
                <ArrowLeft className="w-4 h-4 mr-2" />
                Back to Email Setup
              </Button>
            </Link>
            <div>
              <h1 className="text-2xl font-bold text-gray-900">Pay to Continue Email</h1>
              <p className="text-gray-600">Service interruption notice after 7 days</p>
            </div>
            <Badge variant="secondary" className="ml-auto">Payment</Badge>
          </div>
        </div>
      </div>

      {/* Email Preview */}
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <Card className="max-w-2xl mx-auto">
          <CardHeader className="text-center border-b bg-red-50">
            <div className="w-16 h-16 bg-red-700 rounded-full flex items-center justify-center mx-auto mb-4">
              <XCircle className="w-8 h-8 text-white" />
            </div>
            <CardTitle className="text-xl text-red-800">Service Interruption Notice</CardTitle>
            <p className="text-red-600">Pay to Continue</p>
          </CardHeader>
          <CardContent className="p-6">
            <div className="space-y-4">
              <p className="text-gray-700">Hi {"{"}{"{"} CustomerName {"}"}{"}"},</p>
              
              <div className="bg-red-50 border border-red-200 rounded-lg p-4">
                <div className="flex items-center gap-2 mb-2">
                  <XCircle className="w-4 h-4 text-red-600" />
                  <span className="font-semibold text-red-800">Service Interruption</span>
                </div>
                <p className="text-red-700 text-sm font-medium">
                  Your service has been interrupted due to non-payment of invoice #{"{"}{"{"} InvoiceNumber {"}"}{"}"}  .
                </p>
              </div>
              
              <p className="text-gray-700">
                Your payment of <strong>${"{"}{"{"} InvoiceAmount {"}"}{"}"}  </strong> for {"{"}{"{"} ServiceName {"}"}{"}"}  
                is now 7 days overdue. As a result, your service has been temporarily interrupted.
              </p>
              
              <div className="bg-red-100 border border-red-300 rounded-lg p-4">
                <h4 className="font-semibold text-red-800 mb-2">What happens now:</h4>
                <ul className="text-red-700 text-sm space-y-1">
                  <li>• Your service is currently suspended</li>
                  <li>• Pay now to restore immediate access</li>
                  <li>• Continued non-payment may result in account suspension</li>
                </ul>
              </div>
              
              <p className="text-gray-700">
                To restore your service immediately, please complete your payment using the button below.
              </p>
              
              <div className="flex flex-col sm:flex-row gap-3 pt-4">
                <Button className="flex-1 bg-red-700 hover:bg-red-800">
                  <CreditCard className="w-4 h-4 mr-2" />
                  Pay Now to Restore Service
                </Button>
                <Button variant="outline" className="flex-1">
                  <LogIn className="w-4 h-4 mr-2" />
                  Login to Dashboard
                </Button>
              </div>
              
              <div className="text-xs text-gray-500 pt-4 border-t">
                <p>Need help? Contact our support team immediately for assistance with your payment.</p>
                <p>This is an automated message from Flonnect.</p>
              </div>
            </div>
          </CardContent>
        </Card>
      </div>
    </div>
  );
}